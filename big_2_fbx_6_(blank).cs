...
//			https://docs.microsoft.com/ru-ru/visualstudio/code-quality/ca1006-do-not-nest-generic-types-in-member-signatures?view=vs-2015
				...
				List<List<string>> subMeshFacesStr = new List<List<string>>(); // хранит наборы граней для каждой сабмеши , где [номер_сабмеши][номер_грани]
				SortedSet<int> v_ind = new SortedSet<int>();	//	содержит уникальные индексы вершин для одной сабмеши
				List<List<int>> v_ind_List = new List<List<int>>();	//	исп~ся для выборки вершин, из общего списка, для каждой сабмеши
...
/*sf*/							for ( fi = 0 ; fi < fs_count ; fi++ )	// повторяем необходимое кол-во раз равное кол-ву саб-мешей
										...
										//--------------------------------------------------------------------------------------------------------------
												...
												//	выбираем "уникальные" vertex index из граней сабмеши
												//	чтобы потом выбирать вершины из общего списка для каждой сабмеши
														v_ind.Add(vi1);	v_ind.Add(vi2);	v_ind.Add(vi3_);
												...
												//	это не работает , да и запятая не нужна, потому что отрицательное значение индекса вершины , говорит об окончании
														...
														f_str.Add ( faces ) ; // добавляем строку вида 1 2 -2 в список граней
												}

												subMeshFacesStr.Add ( f_str.ToList() ) ; // добавляем строки в subMeshFacesStr
												f_str.Clear(); // очищаем для следущей сабмеши

												v_ind_List.Add ( v_ind.ToList() ) ;
												v_ind.Clear(); 
...
writePath , 							//	в файл
new List<string> ( ) { 		//	список строк
@"	Vertices: *" + v_ind_List[smi].Count + @" { a: "
});

for ( int w = 0 ; w < v_ind_List[smi].Count ; w++ )
{
		AppendAllTextToObjFile ( 	//	метод пишет
		writePath , 							//	в файл
		new List<string> ( )			//	список строк
		{
		@"	" + v_str[v_ind_List[smi][w]] + ","
		});
}
